/**
 * Project detection using check-my-toolkit's `cm projects detect` command.
 * Detects projects (including monorepo packages) that are missing check.toml.
 */
...
/**
 * Expected JSON output format from `cm projects detect --format json`
 */
...
/**
 * Detect projects in a repository that are missing check.toml configuration.
 * Uses the `cm projects detect` command from check-my-toolkit.
 *
...
export function detectMissingProjects(repoPath: string): MissingProject[] {
  try {
    const result = execSync(
      "cm projects detect --format json --missing-config",
      {
        cwd: repoPath,
        encoding: "utf-8",
        timeout: CM_TIMEOUT,
 ...
 *
 * @param repoPath - Path to the repository root
 * @returns Full cm projects output or null if unavailable
 */
export function detectAllProjects(repoPath: string): CmProjectsOutput | null {
  try {
    const result = execSync("cm projects detect --format json", {
      cwd: repoPath,
      encoding: "utf-8",
      timeout: CM_TIMEOUT,
      stdio: ["pipe", "pipe", "pipe"],
