...
describe("dependency-changes", () => {
  ...
  describe("detectDependencyChanges", () => {
    ...
    it("detects deleted dependency file", () => {
      ...
      expect(result.hasChanges).toBe(true);
      expect(result.changes).toHaveLength(1);
      expect(result.changes[0].file).toBe("check.toml");
    ...
    it("detects multiple changes and groups by check type", () => {
      ...
      expect(result.alwaysTrackedChanges[0].file).toBe("check.toml");
    });
  ...
  describe("edge cases", () => {
    ...
    it("handles nested dependency files", () => {
      ...
      // Add nested check.toml (monorepo style)
      ...
      const result = detectDependencyChanges(testDir, {
        baseCommit,
        dependencies: createMockDependencies({
          files: ["packages/api/check.toml"],
          alwaysTracked: ["packages/api/check.toml"],
        }),
      });

      expect(result.hasChanges).toBe(true);
      expect(result.changes[0].file).toBe("packages/api/check.toml");
    });
