/**
 * Integration with check-my-toolkit's `cm dependencies` command.
 *
...
/**
 * Schema for validating cm dependencies JSON output.
 */
...
/**
 * Raw output from cm dependencies command.
 */
...
export interface GetDependenciesResult {
  ...
  /** Files always tracked (workflows, check.toml, etc.) */
...
/**
 * Parse and validate cm dependencies JSON output.
 */
...
/**
 * Build the cm dependencies command with options.
 */
...
function extractErrorMessage(error: unknown): string {
  ...
  if (error.message.includes("check.toml")) {
    return "no check.toml found";
  }
  return error.message.slice(0, 100);
}

/**
 * Execute the cm dependencies command and return the result.
 */
